...
namespace Jellyfin.Plugin.LDAP_Auth
{
    ...
    internal sealed class LdapAuthHandler(ILogger logger, string dn, string password) : ILdapAuthHandler
    {
        private readonly LdapAuthProvider _provider = new LdapAuthProvider(dn, Encoding.UTF8.GetBytes(password));

        /// <inheritdoc />
        public LdapAuthProvider GetAuthProvider(string host, int port)
        {
            logger.LogDebug("Referred to {Host}:{Port}. Trying bind as user {Dn}", host, port, _provider.Dn);
            return _provider;
        }
    }
}
